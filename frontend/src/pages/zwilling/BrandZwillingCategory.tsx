  ...
  const fetchProductsBySlug = async (
    ...
    try {
      ...
      const res = await api.get(
        `/categories/${category}?page=${page}&size=${pagination.size}&sortBy=price&sortOrder=${sortOrder}&brand=zwilling`
      );

      const fetchedProducts =
        res.data.content.filter(
          (item: IProduct) => item.brand.toLocaleLowerCase() === "zwilling"
        ) || [];
    ...
    <div className="custom-container">
      ...
      <div className="pt-10 pb-5">
        ...
        <div className="flex items-center gap-3">
          ...
          /
          <Link to={"/catalog/zwilling"} className="uppercase">
            ZWILLING
          </Link>
          /<p>{categorys?.name}</p>
        </div>
              ...
              >
                {loading
                  ...
                  : categorys?.subcategories
                      .filter((item) =>
                        item.name.toLocaleLowerCase().includes("zwilling")
                      )
                      .map((item, index: number) => (
                        <SwiperSlide key={index} className="py-4">
                          <Link
                            to={`/catalog/zwilling/${categorys.slug}/${item.slug}`}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
            {products
              //   .filter(
              //     (product) => product.brand.toLocaleLowerCase() === "zwilling"
              //   )
              .map((product) => (
                <div key={product.id}>
...
export default BrandZwillingCategory;
