...
const BrandStaubCategory = () => {
  ...
  const fetchProductsBySlug = async (
    ...
    try {
      ...
      const res = await api.get(
        `/categories/${category}?page=${page}&size=${pagination.size}&sortBy=price&sortOrder=${sortOrder}&brand=staub`
      );

      const fetchedProducts =
        res.data.content.filter(
          (item: IProduct) => item.brand.toLocaleLowerCase() === "staub"
        ) || [];
  ...
  const renderText = () => {
    switch (category) {
      ...
      case "posuda":
        return "Подберите подходящую посуду на любой случай в интернет-магазине STAUB. Наши изделия идеально дополнят вашу кухню. Сковороды, кастрюли, кувшины, кокоты и многое другое ждут вас. Откройте их для себя. Приятных покупок!";
      case "aksessuary":
      ...
      case "mir-krasoty":
        return "С STAUB Beauty окунитесь в мир элегантности и точности маникюра, педикюра и ухода за кожей, который передается из поколения в поколение. Наши продукты — ваш ежедневный спутник красоты и ухода, созданные для вас на всю жизнь.";
      case "tehnika":
        return "Инновации заложены в ДНК STAUB. С 1731 года нашей целью было разрабатывать передовые продукты в соответствии с самыми высокими стандартами качества и постоянно совершенствовать их. Именно с этой идеей мы создавали каждое устройство STAUB Enfinigy с нуля и совершенствовали каждую деталь.";
      case "hranenie":
      ...
      default:
        return "Откройте для себя качество и инновации с продукцией STAUB. Мы предлагаем широкий ассортимент товаров для кухни, дома и ухода за собой — от ножей и посуды до аксессуаров, техники и решений для хранения. Найдите то, что сделает ваш повседневный опыт ещё лучше.";
    }
  };
    ...
    <div className="custom-container">
      ...
      <div className="pt-10 pb-5">
        ...
        <div className="flex items-center gap-3">
          ...
          /
          <Link to={"/catalog/staub"} className="uppercase">
            STAUB
          </Link>
          /<p>{categorys?.name}</p>
        </div>
              ...
              >
                {loading
                  ...
                  : categorys?.subcategories
                      .filter((item) =>
                        item.name.toLocaleLowerCase().includes("staub")
                      )
                      .map((item, index: number) => (
                        <SwiperSlide key={index} className="py-4">
                          <Link
                            to={`/catalog/staub/${categorys.slug}/${item.slug}`}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
            {products
              //   .filter(
              //     (product) => product.brand.toLocaleLowerCase() === "staub"
              //   )
              .map((product) => (
                <div key={product.id}>
...
export default BrandStaubCategory;
